...
$(function () {
    ...
    ///说明：
    ///     城市联动选择（插件）
    var citylinkageSelect = function () {
        //定义默认参数
        this.defaultParams = {
            provincesId: "",    //省ID
            cityId: "",         //市ID
    ...
    citylinkageSelect.prototype = {
        constructor: citylinkageSelect,
        ...
        _init: function () {
            this.initData();
            this.registerProvincesChange();
            this.registerCityChange();
        },

        //初始化省数据
        initData: function () {
            var provincesId = this.options.provincesId;
            var cityId = this.options.cityId;
            var countyId = this.options.countyId;

            //省级数据
            this.setSelectData("0", provincesId);

            //市级数据
            this.setSelectData("1", cityId);

            //区/县数据
            this.setSelectData("2", countyId);
        },

        /****************************(注册事件 begin )******************************/

        registerProvincesChange: function () {
            var provincesId = this.options.provincesId;
            var handleEvent = commonOp.delegate(this._handlerProvincesChange, this);
            $("#selectpick_" + provincesId).click(handleEvent);
        },

        registerProvincesLIClick: function () {
            var provincesId = this.options.provincesId;
            var cityId = this.options.cityId;
            var countyId = this.options.countyId;
            $(".selectpick_ul_" + provincesId + " li").click(function () {
                var currId = $(this).find("label:first").html();
                citylinkageSelect.prototype._handlerProvincesLIClick(currId, cityId, countyId);
            });
        },

        registerCityChange: function () {
            var cityId = this.options.cityId;
            var handleEvent = commonOp.delegate(this._handlerCityChange, this);
            $("#selectpick_" + cityId).click(handleEvent);
        },

        registerCityLIClick: function () {
            var cityId = this.options.cityId;
            var countyId = this.options.countyId;
            $(".selectpick_ul_" + cityId + " li").click(function () {
                var currId = $(this).find("label:first").html();
                citylinkageSelect.prototype._handlerCityLIClick(currId, countyId);
            });
        },
        ...
        _handlerProvincesChange: function () {
            this.registerProvincesLIClick();
        },

        _handlerProvincesLIClick: function (currId, cityId, countyId) {

            //获取市级信息
            citylinkageSelect.prototype.setSelectData(currId, cityId);

            //获取区/县级信息
            var keyofcitydata = citylinkageSelect.prototype.getkeyofdata(currId);
            var firstCityKey;//获取第一个key
            $.each(cityItems[keyofcitydata], function (key, item) {
                firstCityKey = key;
                return false;
            });
            citylinkageSelect.prototype.setSelectData(firstCityKey, countyId);
        },

        _handlerCityChange: function () {
            this.registerCityLIClick();
        },

        _handlerCityLIClick: function (currId, countyId) {

            //获取区/县级信息
            citylinkageSelect.prototype.setSelectData(currId, countyId);
        },
        ...
        //获取key
        getkeyofdata: function (id) {
            var keyOfData;
            $.each(cityItems, function (key, item) {
                var keylist = key.split(',');
                var keylistLen = keylist.length;
                if (keylistLen == 1 && $.trim(keylist[keylistLen - 1]) == id) {
                    keyOfData = key;
                    return false;
                }
                if ($.trim(key) != id && $.trim(keylist[keylistLen - 1]) == id) {
                    keyOfData = key;
                    return false;
                }
                return true;
        ...
        //  lastKey:上级key
        //  selectId：select控件Id
        setSelectData: function (lastKey, selectId) {
            var keyofcountydata = citylinkageSelect.prototype.getkeyofdata(lastKey);
            var countyHtml = "";
            var firstKey;
            var firstItem;
            var countyIndex = 0;
            $.each(cityItems[keyofcountydata], function (key, item) {
                if (countyIndex == 0) {
                    firstKey = key;
                    firstItem = item;
                }
                countyIndex++;
                countyHtml += "<option value='" + key + "'>" + item + "</option>";
            });
            $("#selectpick_span_" + selectId).html(firstItem);
            $("#" + selectId).empty();
            $("#" + selectId).append(countyHtml);
        }

        /****************************(  调用方法 end )******************************/
    };

    $.citylinkageSelect = new citylinkageSelect();
